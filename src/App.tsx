...
import { 
  ...
  Package, Home, ChevronRight, Tag, Layout, Layers, Edit3, Camera, X, Plus, Search, Download, Video
} from 'lucide-react';
    ...
    variants: [
      {
        ...
        video: 'https://assets.mixkit.co/videos/preview/mixkit-man-dancing-under-the-rain-4458-large.mp4',
        subImages: [
...
export default function App() {
  ...
  const updateEditingVariantVideo = (value: string) => {
    const newProduct = {...editingProduct};
    newProduct.variants[editingVariantIdx].video = value;
    setEditingProduct(newProduct);
  };
  ...
  const copyImage = async (url: string) => {
    ...
    try {
      ...
      // We wrap the blob in a ClipboardItem
                      ...
                      <div className="p-4 sm:p-8 flex-1 flex flex-col justify-between space-y-3 sm:space-y-6 overflow-hidden">
                        ...
                        <div className="flex flex-col lg:flex-row items-stretch lg:items-end justify-between gap-3 sm:gap-6">
                          <div className="space-y-2 sm:space-y-3">
                            <span className="text-[8px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Gallery & Video (Click to Copy)</span>
                            <div className="flex gap-1.5 sm:gap-2">
                              {variant.video && (
                                <motion.div 
                                  whileTap={{ scale: 0.92 }}
                                  className="w-8 h-8 sm:w-16 sm:h-16 bg-apple-blue/10 rounded-lg sm:rounded-xl overflow-hidden border border-apple-blue/40 cursor-pointer relative group/vid"
                                >
                                  <video src={variant.video} loop muted playsInline autoPlay className="w-full h-full object-cover" />
                                  <div className="absolute inset-0 flex items-center justify-center bg-black/20">
                                    <Video size={10} className="sm:w-4 sm:h-4 text-white drop-shadow-md" />
                                  </div>
                                </motion.div>
                              )}
                              {(variant.subImages || [null, null, null, null]).slice(0, 4).map((img, i) => (
                                <motion.div 
                                  key={i} 
                                  whileTap={{ scale: 0.92 }}
                                  className="w-8 h-8 sm:w-16 sm:h-16 bg-apple-secondary rounded-lg sm:rounded-xl overflow-hidden border border-apple-border/40 cursor-pointer hover:border-apple-blue transition-all group/sub" 
                                  onClick={() => img && copyImage(img)}
                                >
                              ...
                              <motion.div 
                                whileTap={{ scale: 0.92 }}
                                className="w-8 h-8 sm:w-16 sm:h-16 bg-slate-900 rounded-lg sm:rounded-xl flex items-center justify-center cursor-pointer hover:bg-slate-800 transition-all shadow-lg shadow-slate-900/10" 
                                onClick={() => copyAllSubImages(variant.subImages)}
                              >
                  ...
                  <div className="flex-1 w-full space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                         <Video size={12} className="text-apple-blue" />
                         <input 
                           placeholder="Video URL..." 
                           className="text-[10px] font-medium bg-apple-blue/5 border-none rounded-lg px-2 py-1 outline-none w-32"
                           value={editingProduct.variants[editingVariantIdx].video || ''}
                           onChange={(e) => updateEditingVariantVideo(e.target.value)}
                         />
                      </div>
                    </div>
        ...
        {copyToast && (
          ...
          >
             ...
             <span className="text-xs font-bold tracking-tight">
               {copyToast === 'processing' ? 'Đang xử lý...' : 
                copyToast === 'image' ? 'Đã copy hình ảnh sản phẩm' : 
                copyToast === 'error' ? 'Lỗi khi sao chép' :
                copyToast === 'fallback' ? 'Không hỗ trợ clipboard, đã mở ảnh' : 
                copyToast === 'saved' ? 'Đã cập nhật kho hàng' : 
                copyToast === 'info' ? 'Đã copy thông tin sản phẩm' :
                copyToast === 'no_images' ? 'Không tìm thấy hình ảnh' :
                copyToast === 'all_images' ? 'Đã copy ảnh ghép' :
                copyToast === 'copied' ? 'Đã copy câu trả lời' :
                'Thành công'}
             </span>
          </motion.div>
        )}
